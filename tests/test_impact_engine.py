...
def test_derates_and_unavailable_sets():
    ...
    plan = IsolationPlan(
        plan_id="demo",
        actions=[
            IsolationAction(component_id="source->uA", method="isolate"),
            IsolationAction(component_id="source->uB1", method="isolate"),
            IsolationAction(component_id="source->local1", method="isolate"),
        ],
    )
